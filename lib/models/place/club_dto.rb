class ClubDTO < PlaceDTO
	...
	def initialize(place, char = nil)
		...
		@visitors = CharacterAction.where(:action_id => place.enter_action).where{character_id.not_eq char.id unless char.nil?}.order('id DESC').limit(50).map {|ca| ca.character.dto}
		@bar_factor = 1
	end
end
